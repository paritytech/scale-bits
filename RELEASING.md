...
These steps assume that you've checked out the `scale-bits` repository and are in the root directory of it.
...
5.  Update `CHANGELOG.md` to reflect the difference between this release and last. If you're unsure of
    ...
    Looking at the [closed PRs](https://github.com/paritytech/scale-bits/pulls?q=is%3Apr+is%3Aclosed) can also help to provide details for the changes.
...
9.  If the release was successful, then tag the commit that we released in the `main` branch with the
    ...
    Once this is pushed, go along to [the releases page on GitHub](https://github.com/paritytech/scale-bits/releases)
