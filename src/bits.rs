...
impl Bits {
    ...
    pub fn with_capacity(num_bits: usize) -> Self {
        ...
        Bits {
            storage: Vec::with_capacity(num_bytes),
            bits_in_last_byte: 0
        }
    }
    ...
    pub fn len(&self) -> usize {
        ...
        // stored so far in it.
        let num_bits = (len - 1) * 8 + self.bits_in_last_byte;
    ...
    pub fn push(&mut self, b: bool) {
        ...
        match self.bits_in_last_byte {
            ...
            n => {
                let byte = self.storage
                    .last_mut()
                    .expect("should be a byte");
                *byte = *byte | (bit_val << n);
                self.bits_in_last_byte += 1;
            }
        }
    ...
    pub fn pop(&mut self) -> Option<bool> {
        ...
        *last_byte = *last_byte ^ (1 << right_shift_amount);
...
impl Decode for Bits {
    fn decode<I: codec::Input>(input: &mut I) -> Result<Self, codec::Error> {
        ...
        let bits_in_last_byte = len_bits & 0b111;
        let len = len_bits / 8 + if bits_in_last_byte > 0 { 1 } else { 0 };
        ...
        for _ in 0..len {
            // Assume that byte encoding for Bits is identical to BitVec
            // (we'll test this to make sure) and just copy the bytes as-is.
