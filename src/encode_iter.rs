...
macro_rules! encode_iter_lsb {
    ($name:ident, $ty:ty) => {
        fn $name<I: ExactSizeIterator<Item = bool>>(mut iter: I, out: &mut Vec<u8>) {
            let len = iter.len();
...
mod test {
    ...
    fn assert_encode_all_store_orders(bits: Vec<u8>) {
        ...
        assert_encode_matches_bitvec!(
            b,
            encode_iter_msb0_u16
        );

        let b: BitVec<u32, Msb0> = bits.iter().collect();
        assert_encode_matches_bitvec!(
            b,
            encode_iter_msb0_u32
        );

        let b: BitVec<u64, Msb0> = bits.iter().collect();
        assert_encode_matches_bitvec!(
            b,
            encode_iter_msb0_u64
        );
    }

    #[test]
    fn test_encode_bits() {
        assert_encode_all_store_orders(
            vec![]
        );
        assert_encode_all_store_orders(
            vec![0]
