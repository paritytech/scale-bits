...
use codec::{ Decode, Compact, Error as CodecError };
...
mod test {
    ...
    macro_rules! assert_iter_matches_bits {
        ($bits:expr, $s:ident) => {{
            ...
            let in_bools: Vec<bool> = $bits.into_iter().collect();
            // Does iterator return the same bools?
            let out_bools: Result<Vec<bool>, _> = $s::new(&*bytes)
                .expect("can't parse length")
                .collect();
            assert_eq!(in_bools, out_bools.expect("can't collect bools"));
        }}
    }

    fn assert_iter_bits_all_store_orders(bits: Vec<u8>) {
        let bits: Vec<bool> = bits.into_iter().map(|n| {
            match n {
                0 => false,
                1 => true,
    ...
    #[test]
    fn test_iter_bits() {
        assert_iter_bits_all_store_orders(
            vec![]
        );
        assert_iter_bits_all_store_orders(
            vec![0]
